...
const group = (group = {}, action) => {
	switch(action.type){
		...
		case 'selectScene':
			return Object.assign({}, group, {selectScene: action.payload});
		case 'refreshCommDetail':
			return Object.assign({}, group, {commDetail: action.payload});
		case 'groupCommDetailLoading':
			return Object.assign({}, group, {groupCommDetailLoading: action.payload});
		case 'groupCommLadderMoney': 
			return Object.assign({}, group, {groupCommLadderMoney: action.payload});
		default:
			return group;
	}
}
...
const joinGroupAddress = (joinGroupAddress = initAddress, action) => {
	switch(action.type){
		...
		case 'extractJoinGroupScene':
			if(action.payload === 'poi'){
				return Object.assign({}, $.util.omit(joinGroupAddress, 'province', 'city', 'district', 'addr'));
			}
			if(action.payload === 'logistics'){
